...
public class EncryptUtil {
	...
	public static String getMD5(String text) {
		...
		try {
			...
			byte[] md5Buf = md5.digest(text.getBytes());
			String hexInt = "";
			for (int i = 0; i < md5Buf.length; i++) {
				hexInt = Integer.toHexString(0xFF & md5Buf[i]);
				if (hexInt.length() == 1) { // 高位补0
					hexInt = "0" + hexInt;
				}
				md5Str.append(hexInt);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @return 加密后的字符串
	 */
	public static String encrypt(String sourceStr, String encryptKey) {
		try {
			// 取需要加密内容的utf-8编码。
			byte[] encrypt = sourceStr.getBytes("utf-8");
	 ...
	 * @throws Exception
	 */
	public static String decrypt(String sourceStr, String decryptKey) {
		try {
		    if(StringUtils.isBlank(sourceStr)) {
		        return sourceStr;
		    }
			// base64解码
			byte[] encBuf = Base64.decodeBase64(sourceStr);
			
			// 取密钥和偏转向量
			byte[] key = new byte[8];
			byte[] iv = new byte[8];
			getKeyIV(decryptKey, key, iv);
	...
	public static void main(String[] args) {
		String keyString = "LmMGStGtOpTiAnxuNyvYt54EQ";
		String testString = "4lYoWz_nXu1R";//"Hello Will ~~ $$你好";
		String encryptString = encrypt(testString, keyString);
		System.out.println("encryptString =======> "+encryptString);
		
		System.out.println("return back =======> "+decrypt(encryptString, keyString));
	}
}
